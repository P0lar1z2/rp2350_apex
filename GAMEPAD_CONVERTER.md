...
在 i.MX RT1052 Pro 上新增独立固件 `gamepad_bridge`：OTG2 Host 接收物理键盘与鼠标的 HID 输入，转换为一个标准 USB HID Gamepad 报告，再由 OTG1 Device 发送给 PC。首版面向 PC 版 Apex 的训练场验证。
...
- OTG1：连接 PC，枚举为单接口、全速、1 ms 轮询的标准 HID Gamepad。
- 首版不实现原生 XInput，也不伪装 Xbox、PlayStation 等受认证手柄。EA 的 [PC 无障碍说明](https://www.ea.com/able/resources/apex-legends/pc) 明确列出 Xbox 或其他通用控制器；若特定发行平台仍未识别，可再用 Steam Input 等系统层适配。
- 固件中的 `cafe:1052` 是仅供开发验证的占位 VID/PID；发布产品前必须替换为合法分配的标识。
- 主机平台、认证手柄透传、震动和灯光不在首版范围内。
...
6. 上游报告固定为 13 字节：4 个 16 位摇杆轴、16 个按钮、Hat、LT、RT。
...
- 根工程 `cargo test --lib` 通过，包括移动轴、SOCD、动作键、鼠标缩放/限幅、超时回中、滚轮脉冲、Hat 和释放状态测试。
...
1. 系统识别 `xense / RT1052 KBM Gamepad`，只出现一个标准手柄接口。
2. 系统手柄测试页可观测 4 轴、16 按钮、Hat 和两个独立扳机；中立报告为全轴 0、Hat 8、扳机 0。
