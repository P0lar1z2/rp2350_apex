...
- 默认参数位于 `APEX_DEFAULT_CONFIG`；首次确认 Apex 视角可动后，X/Y 增益由 `192` 提高到 `768`（4 倍），约 30 个单窗口计数达到满幅。仍须按实际 DPI、回报率以及游戏内手柄灵敏度实机校准。
...
- Apex 视角确认开始移动但速度过慢后，X/Y 增益由 `192` 同步提高到 `768`，反死区与 20 ms 回中窗口保持不变。4 倍增益镜像与独立回读 SHA-256 均为 `22a87a087ee6bf5b8350ebaf25df7c4d9d875f9da847f9483bb8d72345b80e9d`，目标已恢复运行。
