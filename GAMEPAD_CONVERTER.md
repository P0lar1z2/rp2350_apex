...
- 默认参数位于 `APEX_DEFAULT_CONFIG`；首次确认 Apex 视角可动后，X 增益由 `192` 逐步提高到 `1024`，Y 增益进一步提高到 `1280`（比 X 高 25%）。X/Y 分别约 23/18 个单窗口计数达到满幅。仍须按实际 DPI、回报率以及游戏内手柄灵敏度实机校准。
...
- Apex 视角确认开始移动但速度过慢后，X/Y 增益由 `192` 先提高到 `768`，再提高到 `1024`；随后按实机手感只把 Y 提高到 `1280`。反死区与 20 ms 回中窗口保持不变。当前镜像与独立回读 SHA-256 均为 `3f19b39b150ed43f9d2c396ce11017dab9db3788dd2920a6612a0f97f84e71a7`，目标已恢复运行。
