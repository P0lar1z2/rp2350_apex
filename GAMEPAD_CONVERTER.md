...
- 默认参数位于 `APEX_DEFAULT_CONFIG`；首次确认 Apex 视角可动后，X/Y 增益由 `192` 逐步提高到 `1024`，约 23 个单窗口计数达到满幅。仍须按实际 DPI、回报率以及游戏内手柄灵敏度实机校准。
...
- Apex 视角确认开始移动但速度过慢后，X/Y 增益由 `192` 先提高到 `768`，再提高到 `1024`；反死区与 20 ms 回中窗口保持不变。当前镜像与独立回读 SHA-256 均为 `20cdf5c5f98dcd2715bfa905ea625819eecae6bfed0618f95ff323acf9a86c8d`，目标已恢复运行。
