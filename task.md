...
- [x] 实机验收两个独立物理 HID 同时接入并聚合为上游复合 HID。
- [ ] 实机验收任意顺序重插及按设备释放。
...
- 多物理设备支持：NXP Host 已去除单一 `s_hidDevice`，最多 6 个 HID 接口分别保存设备句柄和 Hub 端口；Rust 在收齐键盘与鼠标 profile 后创建复合设备，重插时按接口参数与完整 Report Descriptor 重建 Host 槽到上游接口的映射，不依赖插入顺序。
- 双物理设备实测：Logitech `046d:c52b` 暴露 3 个 HID 接口，Lenovo `17ef:62c2` 暴露 2 个 HID 接口；Host 容量扩至 6，OTG1 实测生成 5 接口 High-Speed 复合 HID，Linux 为 5 个接口全部绑定 `usbhid`。确认操作后 Logitech 鼠标、Lenovo 键盘和 Lenovo 鼠标分别完成 476、6、72 个 Interrupt IN 传输，错误数均为 0。
...
- RMII 诊断：RAM 直载遗漏 RUN 时钟初始化时，Boot ROM 低速 AHB/IPG 只能稳定承载 10M，100M 会出现帧长和字节损坏；恢复 528 MHz core/AHB 与 132 MHz IPG 后，100M PHY 本地回环的 60 字节测试帧逐字节一致。
